...
output:
  html_document:
    ...
    toc_depth: 3
    toc_float: true
bibliography: references_cluster.bib
---
...
```{r, echo=FALSE}
knitr::include_graphics('misc/front/clustering.jpg')
```
...
# Overview
Note for Fenna: When loading in count data, do not sure the `load_counts` function but use the designated `read.csv` by uncommenting it. This way, you load in ER or ICU specific count data without needing the metadata (which is not available).
...
We had a problem executing the `fviz_nbclust` function after using the `NbClust` library. A fix was found by altering two functions. Source: @stackoverflow Please execute the code below; after that, everything should work as appropriate.
...
```{r Plotting optimized HC results (3 clusters)}
...
```{r Hierarchical clustering with Euclidean, Ward.D2, and two clusters, fig.cap="Dendrogram for hierarchical clustering with Euclidean, Ward.D2, and two clusters."}
...
```{r Hierarchical clustering with Manhattan, Ward.D2, and two clsuters, fig.cap="Dendrogram for hierarchical clustering with Manhattan, Ward.D2, and two clusters."}
# ward is the same as Ward.D2 for hclust()
...
```{r CDF plot kmeans clustering, fig.cap="Consensus CDF K-means clustering, indicating that k = 2 might be the most appropriate number of clusters. Here, we see a very steep elbow form at the 1.0 on the x-axis."}
...
Next, we extract all significant variables with the function `make_table`. This function makes a latex table highlighting the differences in mean, availability, sample size, and standard error between clusters. We observe that essential indicators of sepsis and oxidative stress are accessible. Such parameters include _First At Ed Neutrophil Count_ (adjusted p-value of 4.692527e-06), _First At Ed Lactate_ (adjusted p-value of 3.182567e-04) (significant for indicating oxidative stress), and _Worst Within 72 Lactate_ (adjusted p-value of 4.075887e-03). However, there are also a couple of variables that are related to SOFA scores, such as _First At Ed Sofa_ (adjusted p-value of 2.427383e-05) and the class variable _Sepsis Severity_ (adjusted p-value of 1.012303e-04).
...
As we did for the ER cohort, we considered looking at the three clustering methods used to determine the optimal number of clusters. We start with hierarchical clustering using Euclidean and Ward (via `ward.D2` since we use `hclust` instead of `agnes`). As we observe from the results, the silhouette is still optimal at two clusters across all MAD-derived gene sets. Additionally, there is a steep decline between 2-4 clusters in WSS, and gap scores are always six clusters.

```{r Optimized HC clustering ICU; checking optimal number of clusters, fig.cap="Figures of all MAD-derived datasets; highlighting the optimal number of clusters for optimized HC clustering. There seems to be a consensus two or three clusters is the most appropriate number of clusters."}
...
```{r Optimized K-means clustering ICU; checking optimal number of clusters, fig.cap="Figures, of all MAD-derived datasets; highlighting the optimal number of clusters for optimized K-means clustering. There seems to be a consensus that two clusters is most appropriate"}
...
```{r Optimized PAM clustering ICU; checking optimal number of clusters, fig.cap="Figures of all MAD-derived datasets; highlighting the optimal number of clusters for optimized PAM clustering. There seems to be a consensus that two or three clusters is the most appropriate number of clusters."}
...
```{r ICU final settings clusters - K-means, fig.cap="K-means on Euclidean distance and two clusters - clusters do not overlap all that much."}
...
```{r ICU final settings clusters - K-means, silhouette, fig.cap="K-means based on Euclidean distance and two clusters. Average silhouette width is 0.19, clusters do not overlap much, but are not compact."}
distances <- dist(mad.list[[4]])
...
```{r ICU final settings clusters - PAM, fig.cap="PAM on Manhattan distance and two clusters - clusters do not overlap all that much."}
...
```{r ICU final settings clusters - PAM, silhouette, fig.cap="PAM based on Manhattan distance and two clusters. Average silhouette width is 0.23, clusters do not overlap that much, but are not that compact."}
...
```{r ICU final settings clusters - HC, fig.cap="Dendrogram for ICU HC clustering for two clusters."}
euclidean_clust <- agnes(dist(mad.list[[4]], 'euclidean'), method = 'ward')
...
```{r PAN clusters distribution with severity 2, fig.cap="PCA plot with the distribution of PAM clusters with severity. Clusters appear heterogeneous."}
plot(pca_clusters[[1]])
```
...
```{r K-means clusters distribution with severity 2, fig.cap="PCA plot with the distribution of PAM clusters with severity. Clusters appear heterogeneous."}
plot(pca_clusters[[3]])
```
...
```{r Overlap between clusters icu, fig.cap="Overlap between clusters groups in severity. There seems to be, in the case of cluster one, a more heterogeneous makeup in the border region. Cluster two seems more homogeneous with High."}
...
```{r Pathway analysis ER setup, fig.cap="Comparison between clusters (ER) based with Reactome, extracting up- and downregulated pathways.", fig.width=10, fig.height=10}
...
```{r ER pathway analysis against healthy controls, fig.cap="Comparison between clusters (ER vs. healthy controls) based with Reactome, extracting up- and downregulated pathways.", fig.height=10, fig.width=10}
...
```{r go plot er vs hc two, fig.cap="Comparison between clusters (ER vs healthy control) based with go cellular component 2018 database, extracting up- and downregulated pathways."}
plot(er_go_hc[[2]])
```

```{r go plot er vs hc three, fig.cap="Comparison between clusters (ER vs healthy control) based with MSigDB Hallmark 2020 database, extracting up- and downregulated pathways."}
plot(er_go_hc[[3]])
```
...
```{r Pathway analysis ICU setup, fig.cap="Comparison between clusters (ICU) based with Reactome, extracting up- and downregulated pathways.", fig.width=10, fig.height=10}
...
```{r go plot icu vs hc two, fig.cap="Comparison between clusters (ICU vs healthy control) based with go cellular component 2018 database, extracting up- and downregulated pathways."}
plot(go_icu_hc[[2]])
```

```{r go plot icu vs hc three, fig.cap="Comparison between clusters (ICU vs. healthy control) based with MSigDB Hallmark 2020 database, extracting up- and downregulated pathways."}
plot(go_icu_hc[[3]])
```
...
```{r Evaluate clusters based on baseline PAM clustering (mito-genes; ICU), fig.cap="Figures of all MAD-derived datasets; highlighting the optimal number of clusters for baseline PAM clustering. There seems to be a consensus that k = 2 or k = 3 is the most appropriate number of clusters. Unfortunately, NbClust is not integrated with PAM."}
pam.mito.icu <- evaluate_clusters('pam', mad.list.icu,  distance = "manhattan")
```

```{r Evaluate clusters based on baseline hierarchical clustering (mito-genes; ICU), fig.cap="Figures of all MAD-derived datasets; highlighting the optimal number of clusters for baseline HC clustering. There seems to be a consensus that k = 2 or k = 3 is the most appropriate number of clusters."}
hc.mito.icu <- evaluate_clusters('hc', mad.list.icu, 
                                 linkage_method = 'ward.D2', skip_nb = T)
```

```{r Evaluate clusters based on baseline K-means clustering (mito-genes; ICU), fig.cap="Figures of all MAD-derived datasets; highlighting the optimal number of clusters for baseline K-means clustering. There seems to be a consensus that k = 2 is the most appropriate number of clusters."}
...
```{r Evaluate clusters based on baseline PAM clustering (mito-genes; ER), fig.cap="Figures of all MAD-derived datasets; highlighting the optimal number of clusters for baseline PAM clustering. There seems to be a consensus that k = 2 or k = 3 is the most appropriate number of clusters. Unfortunately, NbClust is not integrated with PAM."}
pam.mito.er <- evaluate_clusters('pam', mad.list.er, distance = "manhattan")
```

```{r Evaluate clusters based on baseline HC clustering (mito-genes; ER), fig.cap="Figures of all MAD-derived datasets; highlighting the optimal number of clusters for baseline PAM clustering. There seems to be a consensus that k = 2 or k = 3 is the most appropriate number of clusters."}
hc.mito.er <- evaluate_clusters('hc', mad.list.er, linkage_method = 'ward.D2',
                                 skip_nb = T)
```

```{r Evaluate clusters based on baseline K-means (mito-genes) clustering, fig.cap="Figures of all MAD-derived datasets; highlighting the optimal number of clusters for baseline K-means clustering. There seems to be a consensus that k = 2 is the most appropriate number of clusters."}
...
Oyelade, J., Isewon, I., Oladipupo, O. O., Aromolaran, O., Uwoghiren, E., Ameh, F., Achas, M., & Adebiyi, E. (2016). Clustering Algorithms: their application to gene expression data. Bioinformatics and Biology Insights, 10, BBI.S38316. https://doi.org/10.4137/bbi.s38316
Jaskowiak, P.A., Campello, R.J. & Costa, I.G. On the selection of appropriate distances for gene expression data clustering. BMC Bioinformatics 15 (Suppl 2), S2 (2014). https://doi.org/10.1186/1471-2015-15-S2-S2
