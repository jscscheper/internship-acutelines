...
knitr::include_graphics('EDA/sepsis.jpeg')
```
...
This Exploratory Data Analysis (EDA)'s data is retrieved from a previous study \cite{prev-study}.
...
The data consists of the following five datasets:
...
```
As shown in the figures [NUMBER] - [NUMBER], many missing values are present in group-specific features. For example, features about patients located in the ICU have no data about ER patients and healthy controls. Since we are dealing with clinical data, it is vital to understand the context behind these missing values. We talked to Nicole Erler, an expert in the field of biostatistics. \cite{erler} In addition, "just" filling in a zero was also not the way to go, according to Erler. Our data is too challenging to impute quickly. It would be a sufficient half-year project on its own and probably not helpful. We, therefore, leave the missing values as they are for now. We could return later and still handle one of the more significant features.
...
We visualize the results in Table [number] and see that a few features are empty for all cohorts. We can safely delete these features.
...
Figure [number] takes the severity variable as the counter. We see that most samples are between the age group of 51-60 to 71-80.  
...
In figure [number], the age_group is used as the counter. Low severity is more pronounced in younger populations, and almost no one under the age of forty died because of sepsis.
...
It is hard to explain every correlation here, but in most figures ([num]-[num]) above we can conclude that `patient_location` has a good correlation between various sepsis SOFA features (e.g. `sepsis-3`). Also, the class variable `sepsis_severity` has high correlations with other SOFA features (e.g., `first_at_ed_sofa` and `icu_sofa`).. We won't be deleting any of these features.

As a final zoom-in of the metadata set, we looked at the SOFA scores per `sample_location` in figure [number]. When calculating these boxplots, we removed the healthy controls entirely. Then we selected four different, highly correlated features with `sepsis_severity`. We can conclude that out of all the ER cohorts, Australia has the highest overall SOFA scores (not with qSOFA). Also, as expected, ICU cohort has the largest range of SOFA scores (between 15+ and ~ 2).
...
meta.sepsis <- meta.data %>%
  filter(patient_location != 'healthy_control')
...
Additionally, it is standard practice not to do this before filtering out low-expressed genes. After that, we compared gene sets with and without low-expressed genes. Results are depicted in figure [number]. After filtering, the variance increased slightly for both PC1 and PC2, which is a good sign.
...
The following three plots visualize the total variance explained per PC. As we can see in figure [number], the first PC contains more than 15% of the total variance, and the second one as well, and after that, the variance explained per PC decreases to less than 5% per additional PC. Figure [number] cumulatively shows this. The inclusion of 89 PCs explains 80% of the total variance. We summarize both described figures in figure [number], a so-called Pareto plot. This figure shows how the cumulative variance increases per PC via the black-dotted line and stops at around 89 PCs to describe 80% of the total variance. Using this as a dimension reduction effort, we would go from >17.000 genes (/features) to 89 features. We calculate with the `explain_variance` function the amount of PCs we need to explain 80% of the variance.
...
We do a similar process for samples, but now we try to extract samples that contribute best to the first two PCs based on variance. We extracted the top 10 most contributing for the first five PCs. The result is depicted in figure [num]. Most samples come from the Toronto cohort, recognizable from the `col` notation.
...
To conclude our findings, we plot the two PCs into density plots in figures [num] and [num]. Clusters are somewhat evenly large for both PCs.
...
In this section, we will do the above analysis partly again, but now only for mitochondria-related genes. To do this, we use the `biomaRt` library to connect to Ensembl, which is used to retrieve _all_ gene names. \cite{ensembl} We use the Ensembl IDs from the raw counts dataset to collect these gene names. After receiving the gene names, we merge the raw counts dataset with them (`gene_set`). Then, we filter out all non-mitochondrial-related genes by utilizing the `mitochondria.genes` dataset (extracted from GSEA with ID "GO:0005739" \cite{go}), which contains all mitochondria-related genes' Ensembl IDs. As the `dim` function shows, we are now left with 1.669 genes.
...
As we did for all raw genes, we look to see if normalization is necessary by comparing normalized and raw counts by PCA. We see the same result: normalization and batch correction are necessary (`sample_identifier` clusters too well). Results are depicted in figures [num] and [num].
...
Next, we take a look at mitochondria-related top-contributor genes based on PCA. A biplot is depicted in figure [num]. Each gene's contribution is less than we say in the biplot for all genes.
...
Figure [num] shows the gene expression patterns for all top-contributors. Five of the PC1 are upregulated, whereas five others are downregulated. _IDH3G_ is the only gene slightly exhibiting upregulation in the middle of the pack.
...
The violin plots in figure [num] also conclude that batch correction works sufficiently well. All sample locations have similar distributions.
...
\bibitem{prev-study} Baghela, A. et al. "Predicting sepsis severity at first clinical presentation: The role of endotypes and mechanistic signatures". eBioMedicine vol. 75, 103776, January 2022. DOI:https://doi.org/10.1016/j.ebiom.2021.103776
\bibitem{erler} Erler. N: https://www.nerler.com/
\bibitem{ensembl} https://www.ensembl.org/index.html
\bibitem{go} https://www.gsea-msigdb.org/gsea/msigdb/human/geneset/GOCC_MITOCHONDRION
\end{thebibliography}
